<!DOCTYPE html>
<html>

<head>
    <title>Cetak Surat {{$letters->category}}</title>
</head>

<body>
    <table border="0" cellspacing="0" cellpadding="0" width="443">
        <tbody>
            <tr>
                <td colspan="2" valign="top">
                    <p style="margin-bottom: -34px">
                        <img width="295" height="80" src="{{ public_path('/images/bawaslu.jpeg')}}" />
                        <strong></strong>
                    </p>
                </td>
                <td width="68">
                </td>
            </tr>
            <tr>
                <td colspan="2" valign="top">
                    <p style="margin-bottom: 2px">
                        Jalan Turangga Nomor 25 Bandung 40264
                    </p>
                    <p style="margin-top: 2px">
                        Telepon 022-61504245/7332604
                    </p>
                    <p style="margin-top: -18px; margin-bottom: -20px">
                        Laman:
                        <a href="http://www.bawaslu-jabarprov.go.id/">
                            www.bawaslu-jabarprov.go.id
                        </a>
                    </p>
                </td>
                <hr />
            </tr>
        </tbody>
    </table>
    <h4 style="margin-bottom: 2px" align="center">
        <u>SURAT TUGAS</u>
    </h4>
    <p style="margin-top: 2px" align="center">
        <strong>Nomor: </strong>
        <strong>/</strong>
        <strong>BAWASLU PROV.JB/</strong>
        <strong>SET</strong>
        <strong>/</strong>
        <strong>{{$roman}}</strong>
        <strong>/20</strong>
        <strong>20</strong>
    </p>
    <p align="center">
        <strong></strong>
    </p>
    <p align="center">
        Kepala Sekretariat Badan Pengawas Pemilihan Umum Provinsi Jawa Barat
    </p>
    <table border="0" cellspacing="0" cellpadding="0">
        <tbody>
            <tr>
                <td width="50" valign="top">
                    <p>
                        Menimbang
                    </p>
                </td>
                <td width="27" valign="top">
                    <p>
                        :
                    </p>
                </td>
                <td width="28" valign="top">
                    <p>
                        a.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-bottom: 2px" align="Justify">
                        {{$letters->information_a}}
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; ">
                        b.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px; " align="Justify">
                        {{$letters->information_b}}
                    </p>
                </td>
            </tr>
        </tbody>
    </table>
    <table border="0" cellspacing="0" cellpadding="0">
        <tbody>
            <tr>
                <td width="50" valign="top">
                    <p>
                        Dasar
                    </p>
                </td>
                <td width="27" valign="top">
                    <p>
                        :
                    </p>
                </td>
                <td width="28" valign="top">
                    <p>
                        1.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-bottom: -20px; " align="Justify">
                        Undang-Undang Nomor 7 Tahun 2017 tentang Pemilihan Umum;
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: -10px; margin-bottom: 2px;">
                        2.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: -10px; margin-bottom: -2px;" align="Justify">
                        Peraturan Presiden Nomor 68 Tahun 2018 Tentang Kedudukan,
                        Tugas, Fungsi, Wewenang, Organisasi, dan Tata Kerja
                        Sekretariat Jenderal Badan Pengawas Pemilihan Umum,
                        Sekretariat Badan Pengawas Pemilihan Umum Provinsi dan
                        Sekretariat Badan Pengawas Pemilihan Umum Kabupaten/Kota;
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;">
                        3.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;" align="Justify">
                        Peraturan Bawaslu Nomor 07 Tahun 2019 tentang Organisasi
                        dan Tata Kerja Sekretariat Jenderal Badan Pengawas
                        Pemilihan Umum dan Sekretariat Badan Pengawas Pemilihan
                        Umum Provinsi, Sekretariat Badan Pengawas Pemilihan Umum
                        Kabupaten/Kota, dan Sekretariat Panitia Pengawas Pemilihan
                        Umum Kecamatan;
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;">
                        4.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;" align="Justify">
                        Peraturan Menteri Keuangan Nomor 178/PMK.05/2018 tentang
                        Perubahan atas Peraturan Menteri Keuangan Nomor
                        190/PMK.05/2012 tentang Tata Cara Pembayaran dalam Rangka
                        Pelaksanaan Anggaran Pendapatan dan Belanja Negara;
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;">
                        5.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;" align="Justify">
                        Peraturan Menteri Keuangan Republik Indonesia Nomor
                        78/PMK.02/2019 tentang Standar Biaya Masukan Tahun Anggaran
                        2020
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;">
                        6.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;" align="Justify">
                        Surat Edaran Ketua Badan Pengawas Pemilihan Umum Nomor
                        0221/K.BAWASLU/KU.00.03/VI/2020 tentang Kebijakan Dukungan
                        Administrasi Keuangan dalam Tatanan Normal Baru Produktif
                        dan Aman Corona Virus Desease 2019 (Covid-19)
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; ">
                        7.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-top: -2px; " align="Justify">
                        Surat Pengesahan Daftar Isian Pelaksanaan Anggaran Petikan
                        Tahun Anggaran 2020 Nomor SP DIPA- 115.01.2.686307/2020
                        Tanggal 12 November 2019 Kode <em>Digital Stamp </em>
                        1330-5604-7629-0100 Satuan Kerja Sekretariat Bawaslu
                        Provinsi Jawa Barat.
                    </p>
                </td>
            </tr>
        </tbody>
    </table>
    <h4 style="margin-bottom: -20px; margin-top: -15px;" align="center">
        MENUGASKAN
    </h4>
    <table border="0" cellspacing="0" cellpadding="0">
        <tbody>
            @foreach ($letters->employee as $item)
            <tr>
                <td width="91" valign="top">
                    <p>
                        @if($loop->index === 0)
                        kepada
                        @endif

                    </p>
                </td>
                <td width="27" valign="top">
                    <p>
                        @if($loop->index === 0)
                        :
                        @endif
                    </p>
                </td>


                <td width="28" valign="top">
                    <p>
                        {{ $loop->iteration }}
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-bottom: 0px;">
                        Nama : {{$item->name}}
                    </p>
                    <p style="margin-top: 2px;">
                        Jabatan : {{$item->position}}
                    </p>

                </td>

            </tr>
            @endforeach
            <tr>
                <td width="75" valign="top">
                    <p>
                        Untuk
                    </p>
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p>
                        1.
                    </p>
                </td>
                <td valign="top">
                    <p style="margin-bottom: 2px;" align="Justify">
                        {{substr($letters->information_a, 18)}} Bertempat di {{$letters->place_duty}} pada tanggal
                        {{ showDateTime($letters->start_date, 'l, d F Y') }} s.d  {{ showDateTime($letters->end_date, 'l, d F Y') }}
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;">2.</p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px; margin-bottom: 2px;" align="Justify">
                        Melaporkan hasil pelaksanaan tersebut kepada Kepala Sekretariat Bawaslu Provinsi Jawa Barat
                    </p>
                </td>
            </tr>
            <tr>
                <td width="75" valign="top">
                </td>
                <td width="5" valign="top">
                </td>
                <td width="4" valign="top">
                    <p style="margin-top: 2px;">3.</p>
                </td>
                <td valign="top">
                    <p style="margin-top: 2px;">
                        Dilaksanakan dengan penuh tanggung jawab.
                    </p>
                    <table border="0" cellspacing="0" cellpadding="0">
                        <tbody>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td valign="top">
                                    <p style="margin-bottom: 2px;" align="center">
                                        Dikeluarkan di Bandung,
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td valign="top">
                                    <p style="margin-top: 2px;" align="center">
                                        Tanggal:  {{ showDateTime($letters->start_date, 'F Y') }}
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td width="39" valign="top">
                                    <p align="center">
                                        <strong></strong>
                                    </p>
                                    <p align="center">
                                        <strong>KEPALA SEKRETARIAT,</strong>
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td width="39" valign="top">
                                    <p>
                                        <strong></strong>
                                    </p>
                                    <p>
                                        <strong></strong>
                                    </p>
                                    <p>
                                        <strong></strong>
                                    </p>
                                    <p>
                                        <strong></strong>
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td valign="top">
                                    <h4 style="margin-bottom: 2px;" align="center">
                                        <strong>
                                            <u>Drs. ELIAZAR BARUS, M.Si.</u>
                                        </strong>
                                    </h4>
                                </td>
                            </tr>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td valign="top">
                                    <p style="margin-top: 2px;" align="center">
                                        <strong>NIP. 197406051994021002</strong>
                                    </p>
                                </td>
                            </tr>
                            <tr>
                                <td width="151" valign="top">
                                </td>
                                <td width="39" valign="top">
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </td>
            </tr>
        </tbody>
    </table>
    <p>
        <strong></strong>
    </p>
</body>

</html>